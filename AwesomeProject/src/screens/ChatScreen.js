...
const ChatScreen = () => {
  ...
  const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NmVkMmFmNDBlNGVkMGZhZGI2MGFiMTgiLCJpYXQiOjE3MjcwNzYwMDd9.UZB_78CWgpMZZ8bvD80L3cFTxCzMslGxZUsrFW8LNzU'; // Replace with your actual token
  ...
  const loadMessages = async (pageNumber) => {
    ...
    setLoading(true);
    try {
      const response = await apiClient.get(`conversation/${conversationId}/messages?page=${pageNumber}&limit=10`);
  ...
  const sendMessage = async () => {
    if (message.trim()) {
      ...
      try {
        const response = await apiClient.post(`conversation/${receiverId}/message`, payload);
  ...
  return (
    <View style={styles.container}>
      <FlatList
        data={messages}
        keyExtractor={(item) => item._id || `${item.senderId}_${item.messageContent}`}
        renderItem={({ item }) => {
          ...
          return (
            ...
            >
              <Text style={{ color: isSentByUser ? 'white' : 'black' }}>{item.messageContent}</Text>
            </View>
          );
        }}
